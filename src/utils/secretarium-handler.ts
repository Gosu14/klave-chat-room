...
const gatewaysConfigReducer = (
    ...
    const [cluster, name, endpoint, key] = record as [
        string,
        string,
        string,
        string
...
const printClusterInfo = (): void => {
    ...
    console.info(
        'Klave Chat App now using the following cluster configuration:'
    );
...
const secretariumHandler = {
    initialize: (): void => {
        ...
        fetch(
            ...
            .then((config: any) => {
                ...
                console.info('Klave Chat App now using config.json overrides');
            })
            .catch(() => {
                processClusterConfig(clusterConfigBase);
            });
    ...
    connect: (): Promise<ConnectionInformation> =>
        new Promise((resolve, reject) => {
            ...
            if (cluster && endpoint && handlerStore.currentKey) {
                console.info(
                    'Klave Chat App now using the following gateway:',
                    endpoint
                );
                handlerStore.currentConnection
                    ...
                    .connect(
                        endpoint,
                        handlerStore.currentKey,
                        secretariumHandler.utils.fromBase64(cluster[1].key)
                    )
                    .then(() => {
                        resolve({
                            cluster: cluster[0],
    ...
    disconnect: (hold = false): Promise<void> =>
        new Promise((resolve) => {
            ...
            console.log('HANDLERSTORE', handlerStore);
            resolve();
        }),
    request: (
        dcApp: string,
