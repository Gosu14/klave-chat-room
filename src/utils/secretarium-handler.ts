...
const printClusterInfo = (): void => {
    ...
    console.info('Klave Chat App now using the following cluster configuration:');
    ...
    connect: (): Promise<ConnectionInformation> =>
        new Promise((resolve, reject) => {
            ...
            do {
                ...
                (nextGateway === currentGateway && cluster[1].gateways?.length > 1) ||
                nextGateway < 0 ||
...
if (
    (import.meta.env.NODE_ENV === 'development' ||
        import.meta.env.VITE_APP_SECRETARIUM_GATEWAYS_OVERWRITABLE === 'true') &&
    window
) {
