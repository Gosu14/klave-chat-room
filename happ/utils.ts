import { Context, Ledger, Notifier } from '@klave/sdk';
import { ErrorOutput, UserKey } from './types';
 ...
 * @query
 */
export function isUserRegistered(key: string) : boolean {

    const user = Ledger.getTable(users).get(key);
    if (user.length === 0) {
